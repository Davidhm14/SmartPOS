...
Compila las dos versiones de SmartPOS:
  dist/              SmartPOS Setup 1.0.0.exe          (original con categorías)
  dist-barcode/      SmartPOS Barcode Setup 1.0.0.exe  (abecedario + código de barras)
"""
...
# ══ Contenidos leídos del disco (estado actual = versión ORIGINAL) ══════════
PRODUCTOS_IPC_ORIG_DISK = PRODUCTOS_IPC.read_text(encoding='utf-8')
CARD_ORIG_DISK          = CARD.read_text(encoding='utf-8')
BUILDER_ORIG            = BUILDER_YML.read_text(encoding='utf-8')

# ══ electron-builder para versión BARCODE ═══════════════════════════════════
...
# ══ Contenido BARCODE de productos IPC ═════════════════════════════════════
PRODUCTOS_IPC_BARCODE = PRODUCTOS_IPC_ORIG_DISK.replace(
    "AND p.nombre LIKE ?\n      params.push(`%${filtros.busqueda}%`)",
    "AND (p.nombre LIKE ? OR p.descripcion LIKE ?)\n      params.push(`%${filtros.busqueda}%`, `%${filtros.busqueda}%`)"
)

# ══ Contenido BARCODE de ProductoCard ══════════════════════════════════════
CARD_BARCODE = CARD_ORIG_DISK.replace(
    "{producto.nombre}",
    "{producto.descripcion?.trim() || producto.nombre}"
)

# Alias para la restauración
PRODUCTOS_IPC_ORIG = PRODUCTOS_IPC_ORIG_DISK
CARD_ORIG          = CARD_ORIG_DISK

# ══ Contenido BARCODE de Ventas.jsx (abecedario A-Z) ══════════════════════
VENTAS_BARCODE = """\
...
# ══ Contenido ORIGINAL de Ventas.jsx (con categorías) ══════════════════════
VENTAS_ORIG = """\
